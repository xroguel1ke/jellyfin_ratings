...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  // Force rating icon click to open a popup window (Zen/Firefox-friendly)
  a.addEventListener('click', (e)=>{
    if(!link || link==='#') return;
    // let modified or non-left clicks behave normally
    if(e.button!==0 || e.ctrlKey || e.metaKey || e.shiftKey || e.altKey) return;
    // simulate Alt+Click so Zen opens an in-page popup
    if(!a.dataset.mdblSimulatedAlt){
      e.preventDefault(); e.stopPropagation();
      a.dataset.mdblSimulatedAlt = '1';
      try {
        ...
        requestAnimationFrame(()=>{ delete a.dataset.mdblSimulatedAlt; });
      }
    }
  });
