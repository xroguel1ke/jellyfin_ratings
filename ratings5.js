...
/* Keep header row stable while Jellyfin re-populates chips */
function stabilizePrimaryRow(durationMs=2000){
  try{
    const fallbackFind = ()=>document.querySelector('.itemMiscInfo.itemMiscInfo-primary')||document.querySelector('.itemMiscInfo');
    const primary = (typeof findPrimaryRow==='function' ? findPrimaryRow() : fallbackFind());
    if(!primary) return;
    const apply = ()=>{ try{ removeBuiltInEndsAt(); ensureInlineBadge(); ensureEndsAtInline(); }catch{} };
    apply();
    const mo = new MutationObserver(()=>{ apply(); });
    mo.observe(primary, {childList:true, subtree:true, characterData:true});
    setTimeout(()=>{ try{ mo.disconnect(); }catch{} }, durationMs);
  }catch{}
}
function updateAll(){
  try{
...
// Hook SPA navigations to re-apply header order reliably
(function(){
  const apply=()=>{ try{ ensureInlineBadge(); ensureEndsAtInline(); removeBuiltInEndsAt(); }catch{} };
  const applyStable=()=>{ try{ stabilizePrimaryRow(2000); }catch{} };
  const _ps=history.pushState; history.pushState=function(){ const r=_ps.apply(this, arguments); setTimeout(apply, 0); return r; };
  const _rs=history.replaceState; history.replaceState=function(){ const r=_rs.apply(this, arguments); setTimeout(apply, 0); return r; };
  window.addEventListener('popstate', ()=>setTimeout(apply,0));
  window.addEventListener('hashchange', ()=>setTimeout(apply,0));
})();
updateAll();
setTimeout(()=>{ try{ stabilizePrimaryRow(2500); }catch{} }, 250);

// Stabilize header order after first paint
setTimeout(()=>{ try{ ensureInlineBadge(); ensureEndsAtInline(); removeBuiltInEndsAt(); }catch{} }, 350);
