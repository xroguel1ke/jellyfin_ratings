...
const DEFAULT_DISPLAY = {
  ...
  // replaced old align select with free positioning
  positionX:0,           // 0–100 vw
  positionY:0,           // 0–100 vh
  align:'left',          // kept for backward compat, no longer shown in UI
...
const LOGO_AVG = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" ry="14" fill="%231b1c20"/><path d="M16 40 L26 24 L36 36 L46 20" stroke="%23ffffff" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"/><circle cx="46" cy="20" r="3" fill="%232a6df4"/></svg>';
...
function mdblApplyPositionTo(el){
  const x = (DISPLAY.positionX ?? 0);
  const y = (DISPLAY.positionY ?? 0);
  if (!el) return;
...
/* Average collector — per-container */
function mdblRegisterScore(container, key, normVal){
  if (!container) return;
  if (!container._mdblScores) container._mdblScores = {};
  if (key) container._mdblScores[key] = normVal;
  mdblUpdateAverage(container);
}
function mdblUpdateAverage(container){
  if (!container) return;
  const scores = container._mdblScores || {};
  const vals = Object.entries(scores)
    .filter(([k,v]) => k !== 'avg_all' && typeof v === 'number' && !isNaN(v))
    .map(([k,v]) => v);
  let wrap = container.querySelector('#mdbl-avg-rating');
  if (!wrap){
    wrap = document.createElement('div');
    wrap.id = 'mdbl-avg-rating';
    wrap.style = 'display:inline-flex;align-items:center;margin:0 6px;gap:6px;';
    const img = document.createElement('img');
    img.src = LOGO_AVG;
    img.alt = 'Average (all)';
    img.style = 'height:1.3em;vertical-align:middle;cursor:pointer;';
    img.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); if (window.MDBL_OPEN_SETTINGS) window.MDBL_OPEN_SETTINGS(); });
    const num = document.createElement('span');
    num.style = 'font-size:1em;vertical-align:middle;cursor:pointer;';
    num.title = 'Open settings';
    num.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); if (window.MDBL_OPEN_SETTINGS) window.MDBL_OPEN_SETTINGS(); });
    wrap.append(img, num);
  ...
  num.textContent = DISPLAY.showPercentSymbol ? (avg + '%') : String(avg);
  const col = getRatingColor(DISPLAY.colorBands, DISPLAY.colorChoice, avg);
  num.style.color = col;
  container._mdblScores['avg_all'] = avg;
  container.append(wrap);
}
...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  mdblRegisterScore(container, key, r);
...
`;
  ...
  window.MDBL_APPLY_POSITION = function(x,y){
    const xx = (typeof x==='number')?x:(DISPLAY.positionX||0);
    const yy = (typeof y==='number')?y:(DISPLAY.positionY||0);
...
// Sources
    ...
    dWrap.innerHTML=`
      ...
      <div class="mdbl-row wide">
        ...
        <input type="range" id="d_posX" min="0" max="100" value="${DISPLAY.positionX ?? 0}" class="mdbl-select" style="width:200px">
      </div>
      <div class="mdbl-row wide">
        <span>Vertical position</span>
        <input type="range" id="d_posY" min="0" max="100" value="${DISPLAY.positionY ?? 0}" class="mdbl-select" style="width:200px">
      </div>
      <div class="mdbl-row wide">
    ...
    mdblUpdateSwatches();
    const rx = panel.querySelector('#d_posX');
    const ry = panel.querySelector('#d_posY');
    const live = ()=>{
      const x = rx ? +rx.value : 0;
      const y = ry ? +ry.value : 0;
      DISPLAY.positionX = x; DISPLAY.positionY = y;
      if (window.MDBL_APPLY_POSITION) window.MDBL_APPLY_POSITION(x,y);
    };
    if (rx) rx.addEventListener('input', live);
    if (ry) ry.addEventListener('input', live);
  }
  ...
  panel.querySelector('#mdbl-btn-save').addEventListener('click', ()=>{
    ...
    const _px = panel.querySelector('#d_posX');
    const _py = panel.querySelector('#d_posY');
    prefs.display.positionX = _px ? +_px.value : 0;
    prefs.display.positionY = _py ? +_py.value : 0;
