  ...
  [...document.querySelectorAll('a.emby-button[href*="themoviedb.org/"]')].forEach(a=>{
    ...
    document.querySelectorAll('.itemMiscInfo.itemMiscInfo-primary').forEach(b=>{
      ...
      Object.assign(div.dataset,{type, tmdbId, mdblFetched:'0'}); ref.insertAdjacentElement('afterend',div);
       try{ MDBL_ENSURE_AVG(div); }catch{} try{
        const rect = div.getBoundingClientRect();
        div.dataset.baseX = ((rect.left / window.innerWidth) * 100).toFixed(2);
        div.dataset.baseY = ((rect.top  / window.innerHeight) * 100).toFixed(2);
        if (DISPLAY.posX!==null && DISPLAY.posX!==undefined && DISPLAY.posY!==null && DISPLAY.posY!==undefined){
          div.style.position = 'fixed';
          div.style.left = '0'; div.style.top = '0';
          div.style.transform = `translate(${DISPLAY.posX}vw, ${DISPLAY.posY}vh)`;
          div.style.zIndex = '99999'; div.style.pointerEvents = 'auto';
        }
      }catch{};
    });
  });
...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  const r=Util.round(n), disp=DISPLAY.showPercentSymbol?`${r}

/* === Average + click-to-settings wrap === */
(function(){
  const __append = appendRating;
  appendRating = function(container, logo, val, title, key, link, count, kind){
    __append(container, logo, val, title, key, link, count, kind);
    try{
      // make icon + number open settings
      const added = container.querySelector(`[data-source="${key}"]`);
      if(added){
        const a = added.querySelector('a'); const s = added.querySelector('span');
        if(a){ a.href='#'; a.onclick = e=>{ e.preventDefault(); e.stopPropagation(); if(window.MDBL_OPEN_SETTINGS) window.MDBL_OPEN_SETTINGS(); }; }
        if(s){ s.style.cursor='pointer'; s.onclick = e=>{ e.preventDefault(); e.stopPropagation(); if(window.MDBL_OPEN_SETTINGS) window.MDBL_OPEN_SETTINGS(); }; }
      }
      // collect for average
      const n = Util.round(Util.normalize(val,key));
      if(isFinite(n)){
        if(!container.__vals) container.__vals = [];
        container.__vals.push(n);
      }
      if(window.MDBL_UPDATE_AVG) window.MDBL_UPDATE_AVG(container);
    }catch(e){}
  };

  window.MDBL_ENSURE_AVG = (container)=>{
    if(!ENABLE_SOURCES.average) return;
  ...
  window.MDBL_UPDATE_AVG = (container)=>{
...
// Sources
    ...
    dWrap.innerHTML=`
      ...
      <div class="mdbl-row wide">
        ...
        <div class="grid-right" style="gap:10px; justify-content:flex-end;">
          <input type="range" id="d_posX" min="0" max="100" step="1" value="${(DISPLAY.posX ?? (document.querySelector('.mdblist-rating-container')?.dataset.baseX || 0))}" style="width:140px">
      ...
      <div class="mdbl-row wide">
        ...
        <div class="grid-right" style="gap:10px; justify-content:flex-end;">
          <input type="range" id="d_posY" min="0" max="100" step="1" value="${(DISPLAY.posY ?? (document.querySelector('.mdblist-rating-container')?.dataset.baseY || 0))}" style="width:140px">
  ...
  panel.querySelector('#mdbl-btn-save').addEventListener('click', ()=>{
    ...
    const clamp01=(v)=>{ v=parseInt(v||'0',10); if(!isFinite(v)) v=0; return Math.max(0, Math.min(100, v)); };
prefs.display.posX = clamp01(panel.querySelector('#d_posX')?.value ?? panel.querySelector('#d_posX_num')?.value ?? 0);
prefs.display.posY = clamp01(panel.querySelector('#d_posY')?.value ?? panel.querySelector('#d_posY_num')?.value ?? 0);
...
window.MDBL_APPLY_POS = function(x,y){
  try{
    const xv = Math.max(0, Math.min(100, parseFloat(x)));
    const yv = Math.max(0, Math.min(100, parseFloat(y)));
    document.querySelectorAll('.mdblist-rating-container').forEach(div=>{
      div.style.position = 'fixed';
      div.style.left = '0'; div.style.top = '0';
      div.style.transform = `translate(${xv}vw, ${yv}vh)`;
      div.style.zIndex = '99999'; div.style.pointerEvents = 'auto';
    });
  }catch{}
};
