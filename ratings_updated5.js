...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  s.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); try{ if(window.MDBL_OPEN_SETTINGS){ window.MDBL_OPEN_SETTINGS(); } else { const pnl=document.getElementById('mdbl-panel'); if(pnl){ pnl.style.display='block'; } else { try{ document.dispatchEvent(new CustomEvent('mdbl-open-settings')); }catch{} } } }catch{}; });
...
====================================================== */
/* Global safe open helper */
window.MDBL_OPEN_SETTINGS = window.MDBL_OPEN_SETTINGS || (function(){
  try{ document.dispatchEvent(new CustomEvent('mdbl-open-settings')); }catch{}
});
(function settingsMenu(){
  ...
  #mdbl-panel header{position:sticky;top:0;background:rgba(22,22,26,0.96);padding:12px 16px;border-bottom:1px solid rgba(255,255,255,0.08);
    display:flex;align-items:center;gap:8px;cursor:move; z-index:2;background:rgba(22,22,26,0.98);backdrop-filter:blur(8px);z-index: 999; background: rgba(22,22,26,0.98); backdrop-filter: blur(8px); box-shadow: 0 1px 0 rgba(255,255,255,0.08);}
  ...
  #mdbl-panel .mdbl-section{position:relative; z-index: 1; padding:12px 16px;display:flex;flex-direction:column;gap:10px}
  #mdbl-panel .mdbl-subtle{color:#9aa0a6;font-size:12px}

  /* unified two-column grid for perfect checkbox alignment */
  #mdbl-panel .mdbl-row,
  #mdbl-panel .mdbl-source{position:relative; z-index: 1; 
    display:grid;grid-template-columns: 1fr var(--mdbl-right-col);
    align-items:center;gap:10px;padding:8px 10px;border-radius:12px
  }
  #mdbl-panel .mdbl-row{position:relative; z-index: 1; background:transparent;border:1px solid rgba(255,255,255,0.06)}
  ...
  .mdbl-num{width:5ch; min-width:5ch;text-align:right;margin-left:6px;}
...
// Sources
      ...
      <div class="mdbl-grid">
        ...
        <div class="grid-row">
          ...
          <div class="grid-right">
            ...
            <span class="sw" id="sw_orange"></span>
          </div>
        </div>
        ...
        <div class="grid-row">
          ...
          <div class="grid-right">
            ...
            <span class="sw" id="sw_yg"></span>
          </div>
        </div>

        <div class="grid-row">
          <label>≥ Medium‑green threshold</label>
          <div class="grid-right">
  ...
  // Update swatches on palette change
  ['#col_red','#col_orange','#col_yg','#col_mg'].forEach(id=>{
    const el = panel.querySelector(id);
    if(el) el.addEventListener('change', mdblUpdateSwatches);
  });
