...
function scanLinks(){
  ...
  [...document.querySelectorAll('a.emby-button[href*="themoviedb.org/"]')].forEach(a=>{
    ...
    document.querySelectorAll('.itemMiscInfo.itemMiscInfo-primary').forEach(b=>{
      ...
      div.style=`display:flex;flex-wrap:wrap;align-items:center;gap:6px;position:fixed;left:0;top:0;transform: translate(${DISPLAY.posX}vw, ${DISPLAY.posY}vh);z-index: 99999;pointer-events:auto;padding:2px 4px;box-sizing:border-box;`;
      Object.assign(div.dataset,{type, tmdbId, mdblFetched:'0'}); ref.insertAdjacentElement('afterend',div);
      
      // compute baseline position (match original placement) if no saved sliders
      try{
        const rect = ref.getBoundingClientRect();
        if (DISPLAY.posX===null || DISPLAY.posX===undefined) { div.dataset.baseX = ((rect.left / window.innerWidth) * 100).toFixed(2); }
        if (DISPLAY.posY===null || DISPLAY.posY===undefined) { div.dataset.baseY = ((rect.top  / window.innerHeight) * 100).toFixed(2); }
        const useX = (DISPLAY.posX===null||DISPLAY.posX===undefined) ? (div.dataset.baseX||'0') : DISPLAY.posX;
        const useY = (DISPLAY.posY===null||DISPLAY.posY===undefined) ? (div.dataset.baseY||'0') : DISPLAY.posY;
        div.style.transform = `translate(${useX}vw, ${useY}vh)`;
      }catch{};
    try{ if(ENABLE_SOURCES.average) MDBL_ENSURE_AVG(div); }catch{}
    });
  });
...
/* --- Average badge helpers --- */
(function(){
  function ensureAverageBadge(container){
    if(container.querySelector('[data-source="average"]')) return;
    const wrap=document.createElement('div'); wrap.dataset.source='average';
    wrap.style='display:inline-flex;align-items:center;margin:0 6px;gap:6px;';
    const a=document.createElement('a'); a.href='#'; a.style.textDecoration='none';
    const img=document.createElement('img'); img.src=LOGO.average; img.alt='Average'; img.style='height:1.3em;vertical-align:middle;cursor:pointer;';
    img.title='Average of all enabled sources that returned a rating';
    a.appendChild(img);
    a.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); if(window.MDBL_OPEN_SETTINGS) window.MDBL_OPEN_SETTINGS(); });
    const s=document.createElement('span'); s.textContent=''; s.style='font-size:1em;vertical-align:middle;cursor:pointer;';
  ...
  function updateAverage(container){
    const wrap = container.querySelector('[data-source="average"]');
    if(!wrap) return;
    const s = wrap.querySelector('span');
    const vals = Array.isArray(container.__vals)?container.__vals.filter(v=>isFinite(v)):[];
    if(vals.length){
      const sum = vals.reduce((a,b)=>a+b,0);
      const r = Math.round(sum/vals.length);
      ...
      // no numbers yet: keep icon only
      s.textContent = '';
      const img = wrap.querySelector('img'); if (img) img.style.removeProperty('filter');
    }
  }
  window.MDBL_ENSURE_AVG = (c)=>{ try{ ensureAverageBadge(c); }catch{} };
  window.MDBL_UPDATE_AVG = (c)=>{ try{ updateAverage(c); }catch{} };
})();
...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  // icon (now opens settings)
  const a=document.createElement('a'); a.href='#'; a.style.textDecoration='none';
  const img=document.createElement('img'); img.src=logo; img.alt=title; img.style='height:1.3em;vertical-align:middle;cursor:pointer;';
  ...
  // collect for average
  try{ if(!container.__vals) container.__vals=[]; container.__vals.push(r); }catch{}
  // sort by configured priority
  [...container.children]
    .sort((a,b)=>(RATING_PRIORITY[a.dataset.source]??999)-(RATING_PRIORITY[b.dataset.source]??999))
    .forEach(el=>container.appendChild(el));
  // update average badge if present
  try{ if(window.MDBL_UPDATE_AVG) window.MDBL_UPDATE_AVG(container); }catch{}
}%`:`${r}`;
  if(container.querySelector(`[data-source="${key}"]`)) return;
...
/* --- Live position apply helper --- */
(function(){
  window.MDBL_APPLY_POS = function(x,y){
    try{
      document.querySelectorAll('.mdblist-rating-container').forEach(el=>{
        const bx = el.dataset.baseX, by = el.dataset.baseY;
        let xv = (x===null||x===undefined||x==='') ? (DISPLAY.posX===null||DISPLAY.posX===undefined ? (bx||0) : DISPLAY.posX) : x;
        let yv = (y===null||y===undefined||y==='') ? (DISPLAY.posY===null||DISPLAY.posY===undefined ? (by||0) : DISPLAY.posY) : y;
        xv = Math.max(0, Math.min(100, parseFloat(xv)));
        yv = Math.max(0, Math.min(100, parseFloat(yv)));
        el.style.transform = `translate(${xv}vw, ${yv}vh)`;
      });
    }catch{}
  };
})();
...
(function settingsMenu(){
  ...
  const ICON={ imdb:LOGO.imdb, tmdb:LOGO.tmdb, trakt:LOGO.trakt, letterboxd:LOGO.letterboxd,
    ...
    roger_ebert:LOGO.roger, anilist:LOGO.anilist, myanimelist:LOGO.myanimelist , average:LOGO.average };
  const LABEL={ imdb:'IMDb', tmdb:'TMDb', trakt:'Trakt', letterboxd:'Letterboxd',
    ...
    roger_ebert:'Roger Ebert', anilist:'AniList', myanimelist:'MyAnimeList' , average:'Average' };
  ...
  #mdbl-panel input[type="range"]{ -webkit-appearance:none; width:100%; height:4px; background:rgba(255,255,255,0.18); border-radius:4px; outline:none; }
#mdbl-panel input[type="range"]::-webkit-slider-thumb{ -webkit-appearance:none; width:16px; height:16px; border-radius:50%; background:#2a6df4; border:1px solid rgba(255,255,255,0.25); cursor:pointer; }
#mdbl-panel input[type="range"]::-moz-range-thumb{ width:16px; height:16px; border-radius:50%; background:#2a6df4; border:1px solid rgba(255,255,255,0.25); cursor:pointer; }
...
// Sources
    ...
    dWrap.innerHTML=`
      ...
      <div class="mdbl-row wide">
        ...
        <div class="grid-right" style="gap:10px; justify-content:flex-end;">
          <input type="range" id="d_posX" value="${(DISPLAY.posX??(document.querySelector(\'.mdblist-rating-container\')?.dataset.baseX||0))}" style="width:140px">
          <input type="number" id="d_posX_num" class="mdbl-num" min="0" max="100" value="${(DISPLAY.posX??(document.querySelector(\'.mdblist-rating-container\')?.dataset.baseX||0))}"> %
        </div>
      </div>
      <div class="mdbl-row wide">
        <span>Y position</span>
        <div class="grid-right" style="gap:10px; justify-content:flex-end;">
          <input type="range" id="d_posY" value="${(DISPLAY.posY??(document.querySelector(\'.mdblist-rating-container\')?.dataset.baseY||0))}" style="width:140px">
          <input type="number" id="d_posY_num" class="mdbl-num" min="0" max="100" value="${(DISPLAY.posY??(document.querySelector(\'.mdblist-rating-container\')?.dataset.baseY||0))}"> %
        </div>
      </div>
  ...
  panel.querySelector('#mdbl-btn-save')

    // also reset live sliders to defaults immediately
    try{
      const xr = panel.querySelector('#d_posX'), xn = panel.querySelector('#d_posX_num');
      const yr = panel.querySelector('#d_posY'), yn = panel.querySelector('#d_posY_num');
      if(xr) xr.value = DEFAULT_DISPLAY.posX || 0;
      if(xn) xn.value = DEFAULT_DISPLAY.posX || 0;
      if(yr) yr.value = DEFAULT_DISPLAY.posY || 0;
      if(yn) yn.value = DEFAULT_DISPLAY.posY || 0;
      if(window.MDBL_APPLY_POS) window.MDBL_APPLY_POS(DEFAULT_DISPLAY.posX||0, DEFAULT_DISPLAY.posY||0);
    }catch{}
    ...
    // position sliders
    prefs.display.posX = clamp01(panel.querySelector('#d_posX')?.value||DISPLAY.posX||0);
    prefs.display.posY = clamp01(panel.querySelector('#d_posY')?.value||DISPLAY.posY||0);
