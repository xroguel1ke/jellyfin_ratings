// ==UserScript==
// @name          Jellyfin Ratings (v11.0.0 — Final Merge)
// @namespace     https://mdblist.com
// @version       11.0.0
// @description   Combines the robust scanning engine of ratings-other-user.js with the rich UI/Features of the original ratings.js.
// @match         *://*/*
// ==/UserScript==

console.log('[Jellyfin Ratings] Loading v11.0.0...');

/* ==========================================================================
   1. CONFIGURATION & CONSTANTS (From Original ratings.js)
========================================================================== */
...
function updateEndsAt() {
    ...
    const ratingContainer = document.querySelector('.mdblist-rating-container');
    ...
    if (minutes > 0) {
        ...
        // Insert logic for "Ends At" BEFORE ratings
        ...
        // Ensure ratings are AFTER ends at
        if (ratingContainer && span.nextSibling !== ratingContainer) {
            span.insertAdjacentElement('afterend', ratingContainer);
        }
...
/* ==========================================================================
   5. LINK GENERATION (Restored from Original)
========================================================================== */
...
function renderRatings(container, data, type) {
    ...
    if (data.ratings && data.ratings.length > 0) {
        ...
        if (masterCount > 0) {
            ...
            html += createRatingHtml('master', masterSum / masterCount, wikiUrl, masterCount, 'Master Rating', 'Sources');
        }
...
function fetchRatings(container, id, type, apiMode) {
    ...
    // API logic: use IMDb only if mode is explicit
    ...
    fetch(apiUrl)
        ...
        .catch(e => {
            ...
            updateStatus(container, `API Error`, '#e53935');
        });
}

// === ENGINE: SCAN & INJECT (Using ratings-other-user.js logic) ===
function scan() {
    updateEndsAt();
        ...
        if (m) {
            ...
            if (rawType === 'tv' && isEpisodeLink) {
                ...
                if (CFG.display.episodeStrategy === 'episode') {
                    // Try to find a valid IMDb ID nearby
                    ...
                    if (parent) {
                        ...
                        if (imdbLink) {
                            ...
                            if (im) imdbEpisodeId = im[0];
                        }
                    }
                    
                    if (imdbEpisodeId) {
                        // Found explicit Episode ID -> Fetch specific episode
                        injectContainer(imdbEpisodeId, 'movie', 'imdb'); 
                    } else {
                        // SAFE FALLBACK: If no IMDb ID found, use Series ID immediately.
                        // This prevents the "Err" by not attempting a bad fetch.
                        injectContainer(rawId, 'show', 'tmdb');
                    }
                } else {
                    // Strategy: Series Rating (Standard)
                    injectContainer(rawId, 'show', 'tmdb');
                }
            } else {
...
/* ==========================================================================
   7. SETTINGS UI (Restored from Original)
========================================================================== */
...
function renderMenuContent(panel) {
    ...
    <div class="mdbl-section" id="mdbl-sec-display">
        ...
        <div class="mdbl-row">
            <span>Episode Rating Source</span>
            <select id="d_ep_strat" class="mdbl-select" style="width:160px">
                <option value="series" ${CFG.display.episodeStrategy==='series'?'selected':''}>Series (Stable)</option>
                <option value="episode" ${CFG.display.episodeStrategy==='episode'?'selected':''}>Episode (Individual)</option>
            </select>
        </div>
